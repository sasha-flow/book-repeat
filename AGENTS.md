...
- Project documentation files:
  ...
  - /specs/design.md - design, UX and UI decisions, conventions of the this project that should be followed across the product that should be respected when AI is implementing anything related to design, UX and UI. It should include ONLY cross-cutting design decisions and conventions that apply across the product, but not feature-specific design decisions (those should be in the respective feature files). It should be updated every time a new design convention is introduced or an existing one is updated. Its role is to ensure consistency in design and user experience across the product, and to provide guidance for implementing new features in a way that aligns with the overall design vision.
